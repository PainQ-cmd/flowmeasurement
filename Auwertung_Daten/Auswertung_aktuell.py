...
def main():
    ...
    plt.savefig("cross_correlation_plot.pdf", format='pdf', dpi=300)
    print("Plot 'cross_correlation_plot.png' und 'cross_correlation_plot.pdf' gespeichert.")

    # --- Checks und Bereinigung für lags_ns vor Dichteplot ---
    ...
    # Berechnung von lags_ns_clean
    lags_ns_clean = lags_ns[np.isfinite(lags_ns)]

    # Plot: Scatter-Plot der Kreuzkorrelations-Lags (wissenschaftlich)
    plt.figure(figsize=(8, 4))
    plt.scatter(range(1, len(lags_ns_clean)+1), lags_ns_clean, color='tab:blue', alpha=0.7, s=30)
    plt.title("Kreuzkorrelations-Lags pro Einzelmessung")
    plt.xlabel("Messungsnummer")
    plt.ylabel("Kreuzkorrelations-Lag (ns)")
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.tight_layout()
    plt.savefig("lag_scatter_plot.png", dpi=300)
    plt.savefig("lag_scatter_plot.pdf", format='pdf', dpi=300)
    print("Plot 'lag_scatter_plot.png' und 'lag_scatter_plot.pdf' gespeichert.")

    # Berechnung von lags_ns_clean und lags_ns_plot direkt vor dem Dichteplot
    ...
    plt.savefig("lag_density_plot.pdf", format='pdf', dpi=300)
    print("Plot 'lag_density_plot.png' und 'lag_density_plot.pdf' gespeichert.")
    ...
    plt.savefig("lag_difference_boxplot.pdf", format='pdf', dpi=300)
    print("Boxplot 'lag_difference_boxplot.png' und 'lag_difference_boxplot.pdf' gespeichert.")

    # QQ-Plot der Lags
    plt.figure(figsize=(6, 4))
    import scipy.stats as stats
    stats.probplot(lags_ns_clean, dist="norm", plot=plt)
    plt.title("QQ-Plot der interpolierten Lags")
    plt.xlabel("Theoretische Quantile (Standardnormalverteilung)")
    plt.ylabel("Empirische Quantile der Lags (ns)")
    plt.tight_layout()
    plt.savefig("lag_qq_plot.png", dpi=300)
    plt.savefig("lag_qq_plot.pdf", format='pdf', dpi=300)
    print("Plot 'lag_qq_plot.png' und 'lag_qq_plot.pdf' gespeichert.")
