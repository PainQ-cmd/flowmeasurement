...
(signal<ID>_1.txt & signal<ID>_2.txt). Erzeugt:
  ...
  • allan_deviation_id<ID>.png / .pdf für alle IDs
...
def main():
    ...
    # --- Residuen-Plot: Abweichung vom Sollwert für jede ID ---
    # (nach Referenzsensor-Auswertung, damit ref_mean_by_id verfügbar ist)
    for meas_id in SELECTED_IDS:
        try:
            sollwert = ref_mean_by_id.get(meas_id, None)
            if sollwert is None:
                continue
            ...
            for i in range(n_pts):
                ...
                V = lag * Cphase**2/(2*L)
                flows[i] = A * V * 1e9 * 60
            residuen = flows - sollwert
            plt.figure(figsize=(8, 4))
            plt.scatter(range(1, len(residuen)+1), residuen, color='tab:red', alpha=0.7, s=30, label='Residuum')
            plt.axhline(0, color='black', linestyle='--', linewidth=1.5, label='Sollwert (Nulllinie)')
            plt.title(f"Abweichung vom Sollwert pro Messung (ID {meas_id})")
            plt.xlabel("Messungsnummer")
            plt.ylabel("Abweichung vom Sollwert (µL/min)")
            plt.legend()
            plt.grid(True, linestyle='--', alpha=0.5)
            plt.tight_layout()
            plt.savefig(f"residuen_plot_id{meas_id}.png", dpi=300)
            plt.savefig(f"residuen_plot_id{meas_id}.pdf", format='pdf', dpi=300)
            plt.close()
            print(f"Residuen-Plot für ID {meas_id} gespeichert.")
        except Exception as e:
            print(f"Residuen-Plot für ID {meas_id} konnte nicht erstellt werden: {e}")
    # --- Allan-Deviation für alle IDs ---
    sos = design_bandpass(fs=1.0/dt, lowcut=0.1e6, highcut=5e6, order=4)
    with open("allan_deviation_limes.txt", "w", encoding="utf-8") as limes_file:
        limes_file.write("ID\tLimes Allan-Deviation [µL/min]\n")
        for meas_id in SELECTED_IDS:
            try:
                sig1, sig2 = load_signals(meas_id)
                n_pts = min(len(sig1), len(sig2))
                ...
                for i in range(n_pts):
                    ...
                    V = lag * Cphase**2/(2*L)
                    flows[i] = A * V * 1e9 * 60
                taus, adev = allan_deviation(flows)
                plt.figure(figsize=(7,4))
                plt.loglog(taus * dt, adev, marker='o', color='tab:green')
                plt.title(f"Allan-Deviation der Durchflussraten (ID {meas_id})")
                plt.xlabel("Averaging Time $\\tau$ (s)")
                plt.ylabel("Allan-Deviation (µL/min)")
                plt.grid(True, which='both', ls='--', alpha=0.5)
                plt.tight_layout()
                plt.savefig(f"allan_deviation_id{meas_id}.png", dpi=300)
                plt.savefig(f"allan_deviation_id{meas_id}.pdf", format='pdf', dpi=300)
                print(f"Allan-Deviation-Plot 'allan_deviation_id{meas_id}.png' und 'allan_deviation_id{meas_id}.pdf' gespeichert.")
                plt.close()
                # Grenzwert/Limes berechnen (Mittelwert der letzten 10 Werte, falls genug vorhanden)
                n_limes = min(10, len(adev))
                limes_val = np.mean(adev[-n_limes:]) if n_limes > 0 else float('nan')
                print(f"ID {meas_id}: Limes Allan-Deviation = {limes_val:.4f} µL/min")
                limes_file.write(f"{meas_id}\t{limes_val:.4f}\n")
            except Exception as e:
                print(f"Allan-Deviation für ID {meas_id} konnte nicht berechnet werden: {e}")
                limes_file.write(f"{meas_id}\tFehler\n")
    ...
    # --- 5) Kombinierter Scatter ohne ID-Annotation ---
    if ref_P and own_P:
        plt.figure(figsize=(8,5))
